...
function effectiveProjectIds(filters: TaskFilterState, scopedProjectIds?: string[]) {
  if (!scopedProjectIds?.length) return filters.projectIds
  if (filters.projectIds.length > 0) {
    return filters.projectIds.filter((id) => scopedProjectIds.includes(id))
  }
...
export function PipelineView({
  ...
  useEffect(() => {
    ...
    let nextFilters = searchParamsToTaskFilters(params)
    if (scopedProjectIds?.length) {
      nextFilters = {
        ...nextFilters,
        projectIds: nextFilters.projectIds.filter((id) => scopedProjectIds.includes(id)),
      }
    }
    setTaskFilters(nextFilters)
    setView(parseView(params.get("view")))
    setShowArchived(params.get("archivedOnly") === "true")
  }, [searchParams, scopedProjectIds])
  ...
  const fetchTasks = async (filters: TaskFilterState) => {
    ...
    const projectIds = effectiveProjectIds(filters, scopedProjectIds)
    if (scopedProjectIds?.length) {
  ...
  const fetchFilterOptions = async (projectList: ProjectOption[]) => {
    ...
    if (sprintsRes.ok) {
      ...
      const scopedSet = scopedProjectIds?.length ? new Set(scopedProjectIds) : null
      setFilterSprints(
        list
          .filter((s: { projects?: { id: string }[] }) => {
            if (!scopedSet) return true
            const sprintProjects = Array.isArray(s.projects) ? s.projects : []
            return sprintProjects.some((p) => scopedSet.has(p.id))
          })
    ...
    const projectIds =
      effectiveProjectIds(taskFilters, scopedProjectIds).length > 0
        ? effectiveProjectIds(taskFilters, scopedProjectIds)
  ...
  }, [session, status, scopedProjectIds, initialProjects])
  ...
  const handleFiltersChange = (next: TaskFilterState) => {
    const scopedNext = scopedProjectIds?.length
      ? { ...next, projectIds: next.projectIds.filter((id) => scopedProjectIds.includes(id)) }
      : next
    setTaskFilters(scopedNext)
    syncURL(scopedNext, view, showArchived)
  }
  ...
  const openCreate = () => {
    ...
    if (scopedProjectIds?.length === 1) {
      openCreateForProject(scopedProjectIds[0])
      return
    }
  ...
  const handleArchiveTasks = async (scope: "selected" | "completed", archived: boolean) => {
    ...
    const archiveProjectIds = effectiveProjectIds(taskFilters, scopedProjectIds)
    try {
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden px-5 md:px-8">
      ...
      {emptyWorkspace ? (
        <div className="flex flex-1 items-center justify-center py-16 text-sm text-muted-foreground">
          Nenhum projeto vinculado a este espaço de trabalho.
        </div>
      ) : (
